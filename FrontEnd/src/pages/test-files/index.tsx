  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="dark:bg-dark-800 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <h2 className="dark:text-dark-100 mb-4 text-lg font-semibold">{item ? "Edit Test File" : "Add Test File"}</h2>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="dark:text-dark-300 mb-1 block text-xs font-medium text-gray-600">Name *</label><input {...register("name")} className={inputCls} />{errors.name && <p className="mt-1 text-xs text-red-500">{errors.name.message}</p>}</div>
            <div><label className="dark:text-dark-300 mb-1 block text-xs font-medium text-gray-600">Code *</label><input {...register("code")} className={inputCls} />{errors.code && <p className="mt-1 text-xs text-red-500">{errors.code.message}</p>}</div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="dark:text-dark-300 mb-1 block text-xs font-medium text-gray-600">Unit</label><input {...register("unit")} className={inputCls} placeholder="e.g. mg/dL" /></div>
            <div><label className="dark:text-dark-300 mb-1 block text-xs font-medium text-gray-600">Category</label><input {...register("category")} className={inputCls} placeholder="e.g. Chemistry" /></div>
          </div>
          {errors.root && <p className="text-xs text-red-500">{errors.root.message}</p>}
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="dark:text-dark-300 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-100 dark:hover:bg-dark-700">Cancel</button>
            <button type="submit" disabled={isSubmitting} className="bg-primary-600 hover:bg-primary-700 rounded-lg px-4 py-2 text-sm font-medium text-white disabled:opacity-60">{isSubmitting ? "Saving…" : item ? "Save" : "Create"}</button>
          </div>
        </form>
      </div>
    </div>
...
function ParameterRow({ testFileId, param, onDelete }: { testFileId: string; param: TestFileParameterDto; onDelete: () => void }) {
  ...
  if (editing) return (
    <form onSubmit={handleSubmit(save)} className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 dark:bg-dark-700">
      ...
      <input {...register("unit")} className="w-20 rounded border border-gray-200 px-2 py-1 text-sm dark:bg-dark-800 dark:border-dark-600 dark:text-dark-100" placeholder="Unit" />
      <input {...register("lowerLimit")} type="number" step="any" className="w-20 rounded border border-gray-200 px-2 py-1 text-sm dark:bg-dark-800 dark:border-dark-600 dark:text-dark-100" placeholder="Low" />
      <input {...register("upperLimit")} type="number" step="any" className="w-20 rounded border border-gray-200 px-2 py-1 text-sm dark:bg-dark-800 dark:border-dark-600 dark:text-dark-100" placeholder="High" />
      <button type="submit" disabled={isSubmitting} className="bg-primary-600 rounded px-2 py-1 text-xs text-white">Save</button>
      <button type="button" onClick={() => { setEditing(false); reset(); }} className="dark:text-dark-300 rounded px-2 py-1 text-xs text-gray-500">Cancel</button>
    </form>
  );
  return (
    <div className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-gray-50 dark:hover:bg-dark-700">
      <span className="flex-1 text-sm dark:text-dark-200">{param.parameterName}</span>
      <span className="w-20 text-xs text-gray-400 dark:text-dark-400">{param.unit ?? "—"}</span>
      <span className="w-20 text-xs text-gray-400 dark:text-dark-400">{param.lowerLimit ?? "—"} – {param.upperLimit ?? "—"}</span>
      <button onClick={() => setEditing(true)} className="rounded p-1 text-gray-400 hover:text-gray-700 dark:hover:text-dark-200"><PencilSquareIcon className="size-3.5" /></button>
      <button onClick={onDelete} className="rounded p-1 text-red-400 hover:text-red-600"><TrashIcon className="size-3.5" /></button>
    </div>
  );
}
...
function ParameterSection({ file, onRefresh }: { file: TestFileDto; onRefresh: () => void }) {
  ...
  return (
    <div className="mt-3 space-y-1">
      <div className="flex items-center gap-2 border-b border-gray-100 pb-1 dark:border-dark-600 text-xs font-medium text-gray-400 uppercase tracking-wide px-3">
        <span className="flex-1">Parameter</span><span className="w-20">Unit</span><span className="w-20">Range</span><span className="w-16" />
      </div>
      {params.map(p => <ParameterRow key={p.id} testFileId={file.id} param={p} onDelete={() => deleteParam(p.id)} />)}
      <form onSubmit={handleSubmit(addParam)} className="flex items-center gap-2 px-3 pt-2">
        <input {...register("parameterName")} className="flex-1 rounded-lg border border-dashed border-gray-300 px-2 py-1 text-sm dark:bg-dark-800 dark:border-dark-500 dark:text-dark-100" placeholder="+ Add parameter name" />
        <input {...register("unit")} className="w-20 rounded-lg border border-dashed border-gray-300 px-2 py-1 text-sm dark:bg-dark-800 dark:border-dark-500 dark:text-dark-100" placeholder="Unit" />
        <input {...register("lowerLimit")} type="number" step="any" className="w-20 rounded-lg border border-dashed border-gray-300 px-2 py-1 text-sm dark:bg-dark-800 dark:border-dark-500 dark:text-dark-100" placeholder="Low" />
        <input {...register("upperLimit")} type="number" step="any" className="w-20 rounded-lg border border-dashed border-gray-300 px-2 py-1 text-sm dark:bg-dark-800 dark:border-dark-500 dark:text-dark-100" placeholder="High" />
        <button type="submit" disabled={isSubmitting} className="bg-primary-600 hover:bg-primary-700 flex items-center gap-1 rounded-lg px-3 py-1 text-xs text-white"><PlusIcon className="size-3" />Add</button>
      </form>
    </div>
  );
}
...
export default function TestFilesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await testFileService.getAll({ search: search || undefined, page, pageSize });
      setItems(res.data.data.items); setTotal(res.data.data.totalCount);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="relative">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search test files…"
        ...
        <div className="flex gap-2">
          ...
          {isAdmin && <button onClick={() => setModal("create")} className="bg-primary-600 hover:bg-primary-700 flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium text-white"><PlusIcon className="size-4" />Add Test File</button>}
        </div>
      </div>

      <div className="dark:bg-dark-800 dark:border-dark-600 border-gray-150 overflow-hidden rounded-xl border bg-white shadow-xs">
        {loading ? <div className="py-12 text-center text-sm text-gray-400 dark:text-dark-400">Loading…</div>
        : items.length === 0 ? (
          ...
          <div key={item.id} className="border-b border-gray-100 dark:border-dark-600 last:border-0">
            <div className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-dark-700/50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 dark:text-dark-400">{item.code}{item.category ? ` · ${item.category}` : ""} · {item.parameterCount} parameters</p>
              </div>
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${item.isActive ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400" : "bg-gray-100 text-gray-500 dark:bg-dark-600 dark:text-dark-400"}`}>
                {item.isActive ? "Active" : "Inactive"}
              </span>
              {isAdmin && (
                <div className="flex gap-1">
                  <button onClick={() => openEdit(item)} className="dark:text-dark-300 rounded-lg p-1.5 text-gray-500 hover:bg-gray-100 dark:hover:bg-dark-600"><PencilSquareIcon className="size-4" /></button>
                  <button onClick={() => { setTarget(item); setModal("delete"); }} className="rounded-lg p-1.5 hover:bg-gray-100 dark:hover:bg-dark-600"><TrashIcon className="size-4 text-red-500" /></button>
                </div>
              )}
            </div>
