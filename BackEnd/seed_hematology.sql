...
-- ── CATATAN ──────────────────────────────────────────────────
-- 1. instrument_id di file .res = XN1000 → pastikan Instrument
--    yang kamu buat mempunyai Code = 'XN1000'
--    Verifikasi dulu:
...
--    WBC sudah ada (5DD563C9-175B-428F-837D-A230ACCC49EA)
-- ══════════════════════════════════════════════════════════════
...
VALUES ('E1000001-0000-0000-0000-000000000001', '14FD9894-C0BB-4C0A-8B19-E36EFB00E40A',
        'RBC', 'RBC', '99990.99', 2, '10⁶/µL', NULL, NULL, GETUTCDATE(), NULL, 0);
...
VALUES ('E1000001-0000-0000-0000-000000000007', '14FD9894-C0BB-4C0A-8B19-E36EFB00E40A',
        'PLT', 'PLT', '999990', 8, '10³/µL', NULL, NULL, GETUTCDATE(), NULL, 0);
...
-- WBC (menggunakan TestFileParameterId yang sudah ada)
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000001')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000001',
        'F1000001-0000-0000-0000-000000000001',
        '5DD563C9-175B-428F-837D-A230ACCC49EA',  -- WBC (existing)
        7.00, 0.35, 5.0, 15, '%', GETUTCDATE(), NULL, 0);

-- RBC
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000002')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000002',
        'F1000001-0000-0000-0000-000000000001',
        'E1000001-0000-0000-0000-000000000001',  -- RBC
        4.25, 0.15, 3.5, 6, '%', GETUTCDATE(), NULL, 0);

-- HB
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000003')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000003',
        'F1000001-0000-0000-0000-000000000001',
        'E1000001-0000-0000-0000-000000000002',  -- HB
        11.50, 0.40, 3.5, 7, '%', GETUTCDATE(), NULL, 0);

-- HCT
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000004')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000004',
        'F1000001-0000-0000-0000-000000000001',
        'E1000001-0000-0000-0000-000000000003',  -- HCT
        34.50, 1.20, 3.5, 7, '%', GETUTCDATE(), NULL, 0);

-- MCV
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000005')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000005',
        'F1000001-0000-0000-0000-000000000001',
        'E1000001-0000-0000-0000-000000000004',  -- MCV
        81.00, 2.50, 3.1, 6, '%', GETUTCDATE(), NULL, 0);

-- MCH
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000006')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000006',
        'F1000001-0000-0000-0000-000000000001',
        'E1000001-0000-0000-0000-000000000005',  -- MCH
        27.00, 0.90, 3.3, 6, '%', GETUTCDATE(), NULL, 0);

-- MCHC
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000007')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000007',
        'F1000001-0000-0000-0000-000000000001',
        'E1000001-0000-0000-0000-000000000006',  -- MCHC
        33.00, 0.80, 2.4, 6, '%', GETUTCDATE(), NULL, 0);

-- PLT
IF NOT EXISTS (SELECT 1 FROM QCSampleTargets WHERE Id = 'G1000001-0000-0000-0000-000000000008')
INSERT INTO QCSampleTargets (Id, QCSampleId, TestFileParameterId, Mean, SD, CV, Tea, TeaUnit, CreatedAt, UpdatedAt, IsDeleted)
VALUES ('G1000001-0000-0000-0000-000000000008',
        'F1000001-0000-0000-0000-000000000001',
        'E1000001-0000-0000-0000-000000000007',  -- PLT
        235.00, 14.00, 6.0, 15, '%', GETUTCDATE(), NULL, 0);

-- ══════════════════════════════════════════════════════════════
-- VERIFIKASI (jalankan setelah seeder)
-- ══════════════════════════════════════════════════════════════
-- SELECT p.TestCode, p.ParameterName, p.Sequence
-- FROM TestFileParameters p
-- WHERE p.TestFileId = '14FD9894-C0BB-4C0A-8B19-E36EFB00E40A'
-- ORDER BY p.Sequence;

-- SELECT s.Name, s.LotNumber, s.Level, t.Mean, t.SD, p.TestCode
